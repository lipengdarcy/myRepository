package org.darcy.framework.database;
...
import org.darcy.framework.database.impl.IRowMapperColumnFilter;
...
public interface IDaoSupport<T> {
	...
	/**
	 * 查询多行结果集，可通过{@link IRowMapperColumnFilter}做列过滤
	 * @param sql
	 * @param filter 列过滤器
	 * @param args
	 * @return
	...
	/**
	 * 分页查询多行结果集<br/>通过{@link IRowMapperColumnFilter} 对列过滤
	 * @param sql查询的sql语句
	 * @param pageNo 查询的起始页
	 * @param pageSize 每页数量
	 * @param filter 
	...
	/**
	 * 分页查询，可单独过滤结果集中某一列 {@link IRowMapperColumnFilter }
	 * @param sql 查询的sql语句
	 * @param pageNo 查询的起始页
	 * @param pageSize页数量
	 * @param filter
