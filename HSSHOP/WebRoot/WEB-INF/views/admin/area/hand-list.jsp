...
<form id="pagerForm" method="post" action="${ctx}/admin/area/hand.do">
...
<div class="pageContent">
	...
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add"
				href="${ctx }/admin/area.do?method=toHandAdd&pid=${prid }"
				target="navTab" external="true" title="添加"><span>区域装卸费-添加</span>
			</a></li>
			<li><a class="icon hand-save-table-btn" external="true" title="保存" ><span>保存</span>
			</a></li>
			<li>
			<!-- 区域选择 -->
			<div id="area2" class="cellCon">
				<div seled-name-show=".pshow-name-ele-2"
			...
			<li><a class="icon" rel="page42" target="navTab"
				href="${ctx }/admin/area/hand.do?&prid=${prid }" target="navTab"
				title="区域装卸费"><span>刷新</span> </a></li>
		</ul>
	</div>
		...
		<tbody>
			<c:forEach var="s" items="${shipList}" varStatus="status">
				<tr id="tr_${s.productregionsId }">
					...
					<td><a title="编辑"
						href="${ctx }/admin/area.do?method=toHandEdit&id=${s.productregionsId }"
	...
	<div  cid="${cid }" class="area-float-panel float-panel" id="hs-float-panel-2">
