...
<script type="text/javascript">
	var saveForm= function(){
		...
		var $form = $("#area_form");
		var id = $form.find("#newstype_id_h").val();
		var name = $.trim($form.find("#name").val());
		var order = $.trim($form.find("#order").val());
		$.ajax({
			url: url+'admin/newsMng.do?method=newstypeEdit',
			data:{
					 id : id,
					 name : name,
					 order : order
...
<body>
<h2 class="contentTitle">新闻类型-修改</h2>
<div id="area_form_ctx" class="pageContent">
	<form id="area_form" method="post" action="" class="pageForm required-validate" >
		<input type="hidden" id="newstype_id_h" value="${s.newstypeid }" />
		<div class="pageFormContent nowrap" layoutH="97">
			<table>
				<tr>
					<th>新闻类型名称：</th>
					<td>
						<input id="name" type="text" value="${s.name }" />
					</td>
				</tr>
				<tr>
					<th>排序：</th>
					<td>
						<input id="order" type="text" value="${s.displayorder }" />
					</td>
				</tr>
				<tr>
					<td colspan="2" >
