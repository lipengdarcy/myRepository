...
<script type="text/javascript">
	...
	var saveEditForm= function(){
		...
		$.ajax({
			url: url+'regionprice/handEdit.do',
			data:postData,
			type:'post',
			dataType:'json',
...
<body>
<h2 class="contentTitle">区域装卸费-修改</h2>
		...
		<div class="pageFormContent nowrap" layoutH="97">
			<table>
				...
				<tr>
					<th>起步价:&nbsp;</th>
					<td >						
						<input type="text" name="starthand" value="${proRegion.starthand }"/>
					</td>
				</tr>
				<tr>
					<th>价格区间:</th>
							...
							<c:forEach var="priceItem" items="${priceList }"   varStatus="status">
								<c:if test="${status.index==0 }">
									<tr class="price-table-row">
										<td class="right-td">最小够买量:</td><td><input type='text' name="priceList[n].buyminquan" value="${priceItem.buyminquan }"></td>
										<td>${prounit.unitname }及以上</td>
										<td><input type='text' name="priceList[n].price" value="${priceItem.price }"></td>
										<td>元/${prounit.unitname}</td><td><span class="space-span">&nbsp;</span></td>
									</tr>
								</c:if>
								<c:if test="${status.index!=0 }">
									<tr class="price-table-row">
										<td class="right-td">购买:</td>
										<td><input type='text' name="priceList[n].buyminquan" value="${priceItem.buyminquan }"></td>
										<td>${prounit.unitname}及以上</td>
										<td><input type='text' name="priceList[n].price" value="${priceItem.price }"></td>
				...
				<tr><th>&nbsp;</th><td><span class="table-button add-price-table-row">+增加价格区间</span></td></tr>
				...
				<tr>
					<td colspan="2" class="right-td">
						<span class="btn btn-default" onclick="saveEditForm();">提交</span>
					</td>
				</tr>
			</table>
		</div>
...
<table id="price-table-row-tmp" style="display:none;">
	<tr class="price-table-row">
		<td class="right-td">购买:</td>
		<td><input type='text' name="priceList[n].buyminquan" value=""></td><td>${prounit.unitname}及以上</td>
		<td><input type='text' name="priceList[n].price" value=""></td><td>元/${prounit.unitname}</td>
		<td class="center-td"><span class="table-button space-span del-row-btn">删除</span></td>
	</tr>
</table>
