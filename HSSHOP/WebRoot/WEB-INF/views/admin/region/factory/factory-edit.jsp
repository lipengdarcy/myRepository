		...
		$.ajax({
			...
			success : function(data) {
				if (data.state == 'success') {
					//alert("编辑成功，影响条目：" + data.count);
					hsArtDialog.content("编辑成功！").showModal();
					$("#area_form_ctx").hide();
...
<body>
	<h2 class="contentTitle">品牌产地-编辑</h2>
						...
						<td><select id="brand">
								<c:forEach var="s" items="${brandList }" varStatus="status">
									<option value="${s.attrvalueid }"
										<c:if test="${brand_id==s.attrvalueid }">selected="selected"</c:if>>${s.attrvalue}</option>
								</c:forEach>
						</select></td>
					</tr>
					...
					<tr>
						...
						<td><select id="place">
								<c:forEach var="s" items="${placeList }" varStatus="status">
									<option value="${s.attrvalueid }"
										<c:if test="${place_id==s.attrvalueid }">selected="selected"</c:if>>${s.attrvalue}
									</option>
								</c:forEach>
						</select></td>
					</tr>
