...
<script type="text/javascript">
	var saveForm= function(){
		...
		$.ajax({
			url: url+'regionbrand/brandEdit.do',// 跳转到 action    
			data:{    
					 id:id,
					 regions_id : regionId,
					...
					<td>
						<select id="brand" >
							...
							<option value="${s.attrvalueid }" 
								<c:if test="${proregionbrand.brandId==s.attrvalueid }">selected="selected"</c:if>>${s.attrvalue}</option>
							</c:forEach>
						</select>
					</td>
				</tr>
...
<div id="area_form_success_ctx" class="pageContent" style="display:none;padding-left:20px;">
	<div style="color:green;margin:20px 0;"><h3>编辑成功！</h3></div>
</div>
