		...
		<div class="wrap clearfix">
			...
			<div class="user-content right">
				<form id="form" method="post" action="">
					...
					<h1>订单列表</h1>
					<!-- 经销商买入商品 -->
					<c:if test="${param.roleid=='2' }">
						<ul class="user-content-tab clearfix">
							<li <c:if test="${empty param.orderstate}"> class="active"</c:if>><a
								href="${ctx }/business/queryOrder.do?storeType=${param.storeType }&roleid=${param.roleid}">所有订单</a></li>
							<li <c:if test="${param.orderstate=='0'}"> class="active"</c:if>><a
								href="${ctx }/business/queryOrder.do?storeType=${param.storeType }&roleid=${param.roleid}&orderstate=0">待确认</a></li>
							<li <c:if test="${param.orderstate=='6'}"> class="active"</c:if>><a
								href="${ctx }/business/queryOrder.do?storeType=${param.storeType }&roleid=${param.roleid}&orderstate=6">待发货</a></li>
							<li <c:if test="${param.orderstate=='7'}"> class="active"</c:if>><a
								href="${ctx }/business/queryOrder.do?storeType=${param.storeType }&roleid=${param.roleid}&orderstate=7">已发货</a></li>
						</ul>
					</c:if>
					<!-- 经销商卖出商品 -->
					<c:if test="${param.roleid=='1' }">
						<ul class="user-content-tab clearfix">
							...
							<li <c:if test="${param.orderstate=='6'}"> class="active"</c:if>><a
								href="${ctx }/business/queryOrder.do?storeType=${param.storeType }&roleid=${param.roleid}&orderstate=6">待发货</a></li>
							<li <c:if test="${param.orderstate=='15'}"> class="active"</c:if>><a
								href="${ctx }/business/queryOrder.do?storeType=${param.storeType }&roleid=${param.roleid}&orderstate=15">待收款</a></li>
							<li <c:if test="${param.orderstate=='10'}"> class="active"</c:if>><a
								href="${ctx }/business/queryOrder.do?storeType=${param.storeType }&roleid=${param.roleid}&orderstate=10">待评价</a></li>
						</ul>
					</c:if>
					...
					<table>
						<tbody>
							<tr>
								...
								<c:if test="${param.storeType=='1' &&  param.roleid == 1}">
									<th scope="col" class="table-wd01">收货人</th>
								</c:if>
								<!-- 只显示B2B订单的数量 -->
								<c:if test="${param.storeType=='2' &&  param.roleid == 2}">
							...
							<table>
								<tbody>
									...
									<tr class="table-sline">
										...
										<c:if test="${param.storeType=='1' &&  param.roleid == 1}">
											<td class="table-wd01">${item.consignee}</td>
										</c:if>
										<c:if test="${param.storeType=='2' &&  param.roleid == 2}">
										...
										<td class="table-wd01 order-operate"><a target="_blank"
											...
											</c:if> <c:if test="${param.storeType==1 &&  param.roleid == 2}">
												<a
													onclick="dealAddProductToBuy(${item.boplist[0].pid }, ${item.boplist[0].buycount })"
													class="goshop">再次购买 </a>
											</c:if></td>
									</tr>
								</tbody>
